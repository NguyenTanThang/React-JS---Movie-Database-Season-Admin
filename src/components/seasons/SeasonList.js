...
class SeasonList extends Component {
  ...
  setPreviousURL = (e) => {
    e.preventDefault();
    const {currentSeriesID} = this.props;
    localStorage.setItem("returnURL", this.props.location.pathname)
    this.props.history.push(`/seasons/add/${currentSeriesID}`);
  }

  render() {
    const {setPreviousURL} = this;
    const data = this.props.seasons.map(seasonItem => {
      seasonItem.key = seasonItem._id;
      return seasonItem;
    });
    const columns = [
      {
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <Space>
                <Link to={`/seasons/details/${record._id}`} className="btn btn-primary">
                  <i className="fas fa-eye"></i>
                </Link>
                <Link to={`/seasons/edit/${record._id}`} className="btn btn-warning">
                    <i className="fas fa-pen"></i>
                </Link>
                <DeleteSeason seasonItem={record}/>
            </Space>
          )
        }
      },
    ...
    return (
      <>
        <Link className="btn btn-primary" to={`/seasons/add/${currentSeriesID}`} onClick={setPreviousURL}>Add Season</Link>
